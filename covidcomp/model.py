...
class LogisticRegression(Model):
    ...
    def __init__(self, weights0=None, termination_threshold=1e-8, add_bias_term=True):
        ...
        self._add_bias_term = add_bias_term
    ...
    def fit(self, inputs: ndarray, targets: ndarray) -> ndarray:
        ...
        weights = logistic_regression_fit(
            inputs,
            targets,
            weights0=self._weights0,
            termination_threshold=self._termination_threshold,
            add_bias_term=self._add_bias_term,
        )
    ...
    def predict(
        ...
        return logistic_regression_predict(
            inputs, self._weights, decision_threshold, add_bias_term
        )

    def predict_prob(self, inputs: ndarray) -> ndarray:
        """Predict the probability of given data inputs to have
        a target of 1
        ...
        Returns:
            ndarray: an N-dimension vector of predicted probabilities
        """

        return logistic_regression_prediction_probs(
            inputs, self._weights, self._add_bias_term
        )
...
class L2RegularisedLogisticRegression(LogisticRegression):
    ...
    def __init__(
        ...
        Args:
            ...
            add_bias_term (bool, optional): [description]. Defaults to True.
        """
        super().__init__(weights0, termination_threshold, add_bias_term)
    ...
    def fit(self, inputs: ndarray, targets: ndarray) -> ndarray:
        ...
        update_magnitude = 2 * self._termination_threshold

        # Initialise the identity matrix
        identity_matrix = np.identity(D)

        while update_magnitude > self._termination_threshold:
            ...
            H_inv = np.linalg.inv(
                inputs.T @ R @ inputs + self._lambda * identity_matrix
            )
            # Calculate the gradient
            gradient = inputs.T @ (predicts - targets) + self._lambda * weights
            # update the weights
